...
def ask_groq_for_analysis(df):
    """
    Send a compact dataset profile to Groq and request
    data-quality recommendations.
    The AI is used for analysis/recommendations.
    Actual data manipulation is performed deterministically
    by pandas below.
    """
    ...
    if client is None:
        return {
            ...
            "message": (
                ...
                "Set GROQ_API_KEY to enable AI analysis."
            )
        }
...
def clean_missing_values(df, report):
    ...
    Numerical columns:
        Median
    Text columns:
        Mode when available, otherwise "Unknown"
    Datetime columns:
        No blind interpolation; use the most common date.
    """
    for column in df.columns:
            ...
            report.append(
                {
                    ...
                    "count": missing_count,
                    "action": (
                        f'Filled missing text values with "{replacement}".'
                    )
                }
            )
    return df
def detect_and_fix_dates(df, report):
    """
    Detect columns that appear to contain dates and normalize
    them to pandas datetime values.
    Only columns with a reasonable proportion of successfully
    parsed values are converted.
    """
    for column in df.columns:
        series = df[column]
        # Skip columns already stored as datetime.
        if pd.api.types.is_datetime64_any_dtype(series):
            continue
        # Only investigate object/string columns.
        if not (
        ...
        if len(non_null) == 0:
            continue
        # Try parsing the column as dates.
        parsed = pd.to_datetime(
            series,
            errors="coerce"
        )
        valid_ratio = parsed.notna().sum() / len(non_null)
        # Convert only if most values look like dates.
        if valid_ratio >= 0.80:
            before_invalid = int(
                series.notna().sum() - parsed.notna().sum()
            )
            df[column] = parsed
            report.append(
                {
                    ...
                    "action": (
                        "Converted date values to a standardized "
                        "datetime format."
                    )
                }
            )
                ...
                report.append(
                    {
                        ...
                        "action": (
                            "Some invalid date values could not be parsed "
                            "and were converted to missing values."
                        )
                    }
                )
    return df
    ...
    for column in df.columns:
        ...
        changes = int(
            (
                ...
                != cleaned.fillna("__NULL__").astype(str)
            ).sum()
        )
        if changes > 0:
            df[column] = cleaned
            report.append(
                {
                    ...
                    "count": changes,
                    "action": (
                        "Trimmed whitespace and normalized repeated spaces."
                    )
                }
            )
    return df
def detect_and_fix_outliers(df, report):
    ...
    Extreme values are capped at the lower/upper IQR boundaries
    instead of deleting rows.
    This protects the dataset from losing potentially valuable records.
    """
    for column in df.select_dtypes(include="number").columns:
        ...
        if outlier_count == 0:
            continue
        # Special handling for age-like columns.
        column_lower = str(column).lower()
        if (
            "age" in column_lower
            or column_lower == "years"
        ):
            logical_lower = 0
            logical_upper = 120
            ...
            report.append(
                {
                    ...
                    "count": outlier_count,
                    "action": (
                        f"Detected {outlier_count} suspicious age values "
                        f"and capped them to a logical range "
                        f"({final_lower:.1f}–{final_upper:.1f})."
                    )
                }
            )
        else:
            ...
            report.append(
                {
                    ...
                    "count": outlier_count,
                    "action": (
                        f"Capped {outlier_count} IQR outliers "
                        f"to the statistical boundaries "
...
if uploaded_file is None:
    st.markdown(
        ...
        <div class="info-box">
            ...
            Upload a CSV or Excel file to analyze and automatically
...
with analyze_col2:
    st.caption(
        "DataClean Pro uses Groq's llama-3.1-8b-instant model "
        "to identify potential quality issues and recommend actions."
    )
if analyze_clicked:
    ...
    if result["status"] == "success":
        ...
        st.markdown(
            ...
            <div class="metric-card">
                ...
                <div style="font-size:1.5rem;font-weight:750;margin-top:0.4rem;">
                    {quality}
                </div>
...
# ============================================================
st.markdown(
    '<div class="section-title">Automatic Data Cleaning</div>',
    unsafe_allow_html=True
)
st.markdown(
    """
    DataClean Pro will automatically process your dataset using
    a deterministic pandas-based cleaning pipeline.
    """
)
...
if st.session_state.cleaned_df is not None:
    st.markdown(
        '<div class="section-title">Export Clean Data</div>',
        unsafe_allow_html=True
    )
    cleaned_csv = dataframe_to_csv(
    ...
    st.caption(
        "Your cleaned dataset is exported as a UTF-8 CSV file."
    )
# ============================================================
# FOOTER
# ============================================================
